        ...
        if (response) {
          ...
          const tweetContents = tweets
            .map((tweet) => {
              ...
              const views = result.views?.count || result.tweet?.views?.count || "0";
              const bookmarkCount = result.legacy?.bookmark_count || result.tweet?.legacy?.bookmark_count || 0;
          ...
          const rows = comingTweets.map((current: (typeof tweetContents)[0]) => {
            ...
            tweet["username"] = current.user.name;
            ...
            tweet["views_count"] = Number(current.meta?.views) || 0;
            tweet["bookmark_count"] = Number(current.meta?.bookmarkCount) || 0;
            return tweet;
          });
